...
# MAIN.PY
#****************************************************************************
# THIS FLOW IS HANDLING AN E-INVOiCE USING TICKSTAR LAYER                   *
...
def main(request):
    ...
    try:
        ...
        if sender == "petrolcave":
            ...
            if table_arg == "proforma":
                ...
                template = env.get_template("peppol_compliant_invoice_with_jinja_placeholders_petrol_cave_1.xml")
            elif table_arg == "ad_hoc_invoice":
                print("table_arg is ad_hoc_invoice")
                template = env.get_template("peppol_compliant_invoice_with_jinja_placeholders_petrol_cave_ad_hoc_invoice.xml")
            elif table_arg == "proforma_com":
                print("table_arg is proforma_com")
                template = env.get_template("peppol_compliant_invoice_with_jinja_placeholders_petrol_cave_proforma_com.xml")
        ...
        if sender == "petrolcave":
            if table_arg == "proforma_com":
                ...
                pfc_vat_percent = data.get("TaxCategory_Percent", "0.00%").replace("%", "").strip()
                pfc_vat_exemption_reason_code = data.get("TaxExemptionReasonCode", "no code given")
                pfc_vat_exemption_reason = data.get("TaxExemptionReason", "no reason given")
                invoice_lines = build_invoice_lines(data, vat_category=pfc_vat_cat_id , vat_percentage=pfc_vat_percent, unit_code="C62", vat_exemption_reason_code=pfc_vat_exemption_reason_code,vat_exemption_reason=pfc_vat_exemption_reason)
        ...
        if sender != "do_not_process_subtotals": # This is a placeholder to skip this section for testing, so now do it for all senders except "do_not_process_subtotals"
    ...
    if cleaned_invoice_data["P_invoice_typecode"] == 380: #380 = peppol commercial invoice
    ...
    elif cleaned_invoice_data["P_invoice_typecode"] == 389: #389 = peppol self billed invoice
    ...
    #********************************************************************   
    xml_output = template.render(invoice=cleaned_invoice_data, 
