...
from webhook_utils_1 import post_data_to_appsheet_whc
import uuid
...
def main(request):
    ...
    try:
        ...
        date_list = [d.strip() for d in data["New dates to include Zenphi"].split(",")]
        print("_________________________________________________")
    except Exception:
    ...
    print("JSON REQUEST BODY FOR APPSHEET: ", json.dumps(appsheet_data, indent=2))
    #*********************************************************
    try:
        response = post_data_to_appsheet_whc(table, appsheet_data)    
        print("Status Code from find line in: ", table,":", response.status_code)
    ...
    print("JSON FOR APPSHEET: ", json.dumps(appsheet_data, indent=2))
    #*********************************************************
    try:
        response = post_data_to_appsheet_whc(table, appsheet_data)
    #*********************************************************
